...
int main(int argc, char **argv)
{
    ...
    struct timeval iStart, iEnd, iElaps;
    ...
    gettimeofday(&iEnd, NULL);
    timeval_subtract(&iElaps, &iEnd, &iStart);
    printf("sumMatrixOnHost <<<>>> elapsed %d secs %d ms\n", iElaps.tv_sec, iElaps.tv_usec);
    ...
    printf("sumMatrixOnGPU2D <<<(%d,%d), (%d,%d)>>> elapsed %d secs %d ms\n", grid.x,
           grid.y,
           block.x, block.y, iElaps.tv_sec, iElaps.tv_usec);
